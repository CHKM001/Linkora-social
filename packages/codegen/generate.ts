...
function parseSpec(): {
  ...
  for (const xdrStr of xdrStrings) {
    ...
    if (kind === "scSpecEntryFunctionV0") {
      ...
      const variants = u.cases().map((c: xdr.ScSpecUdtUnionCaseV0) => ({
      ...
      const variants = e.cases().map((c: xdr.ScSpecUdtEnumCaseV0) => ({
        name: c.name().toString(),
      }));
      enums.push({ name, variants });
...
function generateClient(
  ...
  // Generate read methods for functions that return values
  const readMethods = functions.filter((f) => f.outputs.length > 0 && f.outputs[0].type !== "void");
  const writeMethods = functions.filter(
    (f) => f.outputs.length === 0 || f.outputs[0].type === "void"
  );
...
function generateEvents(events: EventType[]): string {
  const lines: string[] = [
    "// Auto-generated by packages/codegen/generate.ts — do not edit manually.",
    "/* eslint-disable */",
    "",
...
function scvAddressVec(addresses: string[]): xdr.ScVal {
  return nativeToScVal(addresses.map(addr => Address.fromString(addr).toScVal()), { type: "vec" });
}
`;
}
...
function main() {
  ...
  const eventsContent = generateEvents(events);
